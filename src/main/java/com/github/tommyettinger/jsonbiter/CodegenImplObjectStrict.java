...
class CodegenImplObjectStrict {
    ...
    public static String genObjectUsingStrict(ClassDescriptor desc) {
         ...
         */
        // === if null, return null
        append(lines, "java.lang.Object existingObj = com.github.tommyettinger.jsonbiter.CodegenAccess.resetExistingObject(iter);");
        ...
        append(lines, "field = com.github.tommyettinger.jsonbiter.CodegenAccess.readObjectFieldAsSlice(iter);");
        if (hasAnythingToBindFrom(allBindings)) {
            append(lines, "switch (field.len()) {");
            append(lines, rendered);
            append(lines, "}"); // end of switch
